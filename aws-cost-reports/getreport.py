...
def generate_xls(tag_values, dates, cost_data, start_date, end_date):
    ...
    # Add annotation as a text box (shape) floating over the worksheet
    try:
        from openpyxl.drawing.text import RichText
        from openpyxl.drawing.text import Paragraph, ParagraphProperties, CharacterProperties
        from openpyxl.drawing.spreadsheet_drawing import (
            AbsoluteAnchor, OneCellAnchor, AnchorMarker, SpreadsheetDrawing
        )
        from openpyxl.drawing.geometry import Point2D, PositiveSize2D
        from openpyxl.drawing.fill import SolidFill
        from openpyxl.drawing.line import LineProperties
        from openpyxl.drawing.shape import ShapeProperties

        # Create annotation text for cost-usage$
...
The amount includes a monthly TAX fee, which is charged against untagged resources on the 1st."""
        ...
        # Add the annotation as a merged cell with formatting (simple approach)
        annotation_row = total_row_idx + 2  # Two rows after the total row

        annotation_cell = ws.cell(row=annotation_row, column=1, value=annotation_text)
        annotation_cell.font = Font(name='Arial', size=10, bold=True, color="333333")
        annotation_cell.alignment = Alignment(wrap_text=True, vertical='top', horizontal='left')
        annotation_cell.fill = PatternFill(start_color="FFFFCC", end_color="FFFFCC", fill_type="solid")
        ...
        # Merge cells for the annotation (3 rows high, full table width)
        ws.merge_cells(
            start_row=annotation_row,
            start_column=1,
            end_row=annotation_row + 2,  # 3 rows high
            end_column=len(dates) + 2  # Full table width
        )

        print("✓ Added annotation for 'cost-usage$' as formatted text box")

    except ImportError as e:
        print(f"Note: Could not import drawing modules: {e}")
        print("Annotation will be added as simple merged cells")

        # Fallback: simple annotation
        annotation_row = total_row_idx + 2
        annotation_cell = ws.cell(row=annotation_row, column=1,
                                  value="Annotation: cost-usage$ = untagged resources total cost")
        annotation_cell.font = Font(bold=True, color="366092")
        ws.merge_cells(start_row=annotation_row, start_column=1,
                       end_row=annotation_row, end_column=3)
